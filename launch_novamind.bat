...
if "%SNIPER_SAMPLES%"=="" set SNIPER_SAMPLES=10000
if "%SNIPER_DATA_PATH%"=="" set SNIPER_DATA_PATH=data\sniper_train.jsonl
if "%HF_MODEL_NAME%"=="" set HF_MODEL_NAME=meta-llama/Llama-2-7b-hf
if "%OUTPUT_ROOT%"=="" set OUTPUT_ROOT=.\checkpoints
...
%PYTHON_BIN% data\sniper_dataset_gen.py --output "%SNIPER_DATA_PATH%" --samples %SNIPER_SAMPLES% --seed 42
...
set /p PROCEED=Tiny pre-training complete. Do you want to proceed to Path 2 (7B LoRA Fine-tuning)? (y/n) 
if /i not "%PROCEED%"=="y" (
